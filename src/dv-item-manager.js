define( [ "qlik","jquery","qvangular","./dv-qlik-crud","./dv-qlik-dialog","text!./style.css","text!./template-toolbar.html","text!./template-table.html"],
function ( qlik,$,qvangular,qCRUD,qDialog,cssContent,toolbarTemplate,tableTemplate) {
	...
	objectList.forEach((obj)=>{
		if(obj['show']==true && obj[sField].value!==null){
			...
			if(!Array.isArray(obj[sField].value)){
				...
				obj[sField].display=`<mark style="background-color:#db94ca;">${sValue}</mark>`;
			}else{
...
function addArrayItem(obj,sField,sTags){
	...
	sTags.split(',').forEach((sTag)=>{
		if(!obj[sField].value.includes(sTag)){
			sDisplayNew=`<mark style="background-color:#db94ca;">${sTag}</mark>`;
...
function findReplace(sMode){
		...
		if(obj[sCurrentField].value!== null){
			...
			if(matches!=null && checkAll(obj)){
				...
				obj.show=true;
					...
					if(sMode=='find'){
						obj[sCurrentField].display=sCurrentValue.replace(myRegExp,`<mark style="background-color:#68b5de;">${matches[0]}</mark>`);
					}else{//replace
						obj[sCurrentField].display=sCurrentValue.replace(myRegExp,`<mark style="background-color:#fab761;">${sReplace}</mark>`);
	...
	if(bHasMatch){renderTableBody()};
}
//-----------------------------------------------------
function saveAll(){
	qDialog.luiConfirm(luiPopover,{element:this,body:'Are you sure you want to update all visible varaibles?',confirm:function(){
...
function removeTableRow(num){
	body.querySelector(`[data-num='${num}']`).remove();
}
//-----------------------------------------------------
...
function cloneObject(obj,param){
	...
	//if numeric repeat current name x number of times. If string split by comma and create copy for each
	if(isNaN(param)){
	//non numeric
	...
	if(obj.show){
		...
		case 'variable':
			qCRUD.deleteVariable(myExtension,obj.name.value).then(function(r){
				if(r.qSuccess){removeTableRow(num)};
			});
		break;
		case 'dimension':
...
//-----------------------------------------------------
function deleteAll(){
	qDialog.luiConfirm(luiPopover,{element:this,body:'Are you sure you want to delete the selected items(s)?',confirm:function(){
		objectList.forEach((obj)=>{
				deleteObject(obj);
		})
		}
	...
	objectList.forEach((obj)=>{
		if(obj['show']==true){
			...
			fields.forEach((field)=>{
				...
				//console.log(field,typeof(obj[field].value),Array.isArray(obj[field].value))
...
function addRowButtons(index){
	...
	for(i=0;i<icons.length;i++){
		...
		btn.className=`lui-button lui-button--${styles[i]} lui-buttongroup__button`;
		btn.setAttribute('data-index',index);
		btn.title=titles[i];
		btn.addEventListener('click',rowButtonClick);
		btn.innerHTML=`<span class="lui-button__icon  lui-icon  lui-icon--${icons[i]}"></span>`;
...
function rowButtonClick(){
	vIndex=this.getAttribute('data-index');
	//console.log(this.getAttribute('data-index'));
	//console.log(this.name)
	
	switch(this.title) {
		case 'Edit':
			qDialog.luiVariableEdit(luiPopover,{element:this})
		break;
		case 'Save':
			qDialog.luiConfirm(luiPopover,
				{
					element:this,
					confirm:function(){updateObject(objectList[vIndex]);},
					header:`Save ${objectList[vIndex].type.value} ${objectList[vIndex].type.display}`,
		...
		case 'Delete':
			qDialog.luiConfirm(luiPopover,
				{
					element:this,
					confirm:function(){deleteObject(objectList[vIndex]);},
					header:`Delete ${objectList[vIndex].type.value} ${objectList[vIndex].type.display}`,
			...
			qDialog.luiCopy(luiPopover,
				{
					...
					confirm:function(name){
						...
						;},
					header:`Copy ${objectList[vIndex].type.value} ${objectList[vIndex].type.display}`,
					body:'Are you sure you want to delete this variable?'
				}
			);
			// code block
		break;
		case 'Debug':
			qCRUD.getProperties(myExtension,objectList[vIndex].type.value,objectList[vIndex].id).then(function(result){console.log(result)})
		break;
	}
}
//-----------------------------------------------------
function refreshObjects(){
	...
	//Variables
	layout.qVariableList.qItems.forEach(v => {
		num+=1
	...
	//Measures
	layout.qMeasureList.qItems.forEach((m) => {
		num++;
		objectList.push(qCRUD.retrieveMeasure(m,num))
	})
	
	//Dimensions
	layout.qDimensionList.qItems.forEach((d) => {
		num++;
		//console.log(d)
...
function createToolbar(){
	...
	toolbar.getElementsByClassName('btnFind')[0].addEventListener('click',function(){findReplace('find')});
	toolbar.getElementsByClassName('btnReplace')[0].addEventListener('click',function(){findReplace('replace')});
...
function clearFields(){
	...
	inputApplyField.value='definition';
...
function getTypeSelections(){
	var vals = [];
	for (var i=0, n=inputTypeCheckboxes.length;i<n;i++) {
		if (inputTypeCheckboxes[i].checked) {vals.push(inputTypeCheckboxes[i].value);}
	}
	return vals;
}
//====================================================================================================//
